...
// Basic Error Handling
const productAddState = new Set(); // For tracking admin adding products
const orderAddState = new Map(); // For tracking active order
const configEditState = new Map(); // For editing ticket config
const clientRegState = new Map(); // For tracking client name and phone number registration
const waiterPickupCodeState = new Map(); // For waiter checkout validation
const exportDateState = new Set(); // For tracking users providing a date for CSV export
const expenseState = new Map(); // For tracking expense registration
const waiterClientSearchState = new Map(); // For waiters searching for clients
// Reusable Admin Keyboard
function getAdminKeyboard() {
    return telegraf_1.Markup.inlineKeyboard([
        [telegraf_1.Markup.button.callback('👥 Gestión de Roles', 'admin_roles')],
        [telegraf_1.Markup.button.callback('👥 Gestión de Clientes', 'admin_clientes')],
        [telegraf_1.Markup.button.callback('🪑 Gestión de Mesas', 'admin_mesas')],
        [telegraf_1.Markup.button.callback('🍔 Gestión de Menú', 'admin_menu')],
        [telegraf_1.Markup.button.callback('🧾 Cuentas Abiertas', 'admin_cuentas')],
        [telegraf_1.Markup.button.callback('📊 Finanzas', 'admin_finanzas')],
        [telegraf_1.Markup.button.callback('🖨️ Configuración del Ticket', 'admin_config')]
    ]);
}
...
function getMainKeyboard(role) {
    const buttons = [];
    if (role === 'ADMIN') {
        buttons.push([telegraf_1.Markup.button.text('📝 Nueva Orden / Ver Mesa'), telegraf_1.Markup.button.text('🥡 Para Llevar')]);
        buttons.push([telegraf_1.Markup.button.text('🛎️ Pedidos de Clientes')]);
        buttons.push([telegraf_1.Markup.button.text('🧾 Cuentas Abiertas'), telegraf_1.Markup.button.text('⚙️ Panel de Administración'), telegraf_1.Markup.button.text('📊 Reporte de Ventas')]);
    }
    else if (role === 'MESERO') {
        buttons.push([telegraf_1.Markup.button.text('📝 Nueva Orden / Ver Mesa'), telegraf_1.Markup.button.text('🥡 Para Llevar')]);
        buttons.push([telegraf_1.Markup.button.text('🛎️ Pedidos de Clientes'), telegraf_1.Markup.button.text('🧾 Cuentas Abiertas')]);
    }
    else if (role === 'CONTADOR') {
        buttons.push([telegraf_1.Markup.button.text('📊 Reporte de Ventas')]);
    }
    else if (role === 'CLIENTE') {
        buttons.push([telegraf_1.Markup.button.text('🛍️ Hacer Pedido')]);
        buttons.push([telegraf_1.Markup.button.text('📋 Mis Pedidos'), telegraf_1.Markup.button.text('📖 Ver Menú')]);
    }
    buttons.push([telegraf_1.Markup.button.text('❓ Ayuda'), telegraf_1.Markup.button.text('🚪 Salir')]);
    return telegraf_1.Markup.keyboard(buttons).resize();
}
// Generate Ticket Image Function
async function generateTicketImage(order, config) {
    ...
    try {
        ...
        if (!user) {
            ...
            if (isFirst) {
                ...
                await ctx.reply(`¡Bienvenido ${firstName}! ¿Cómo deseas registrarte?`, telegraf_1.Markup.keyboard([
...
bot.hears('💼 Soy Empleado', async (ctx) => {
    ...
    try {
        ...
        await ctx.reply('Tu cuenta está en estado PENDIENTE. Un administrador debe aprobarte.', telegraf_1.Markup.removeKeyboard());
        // Notify Admins
        ...
        for (const admin of admins) {
            try {
                await ctx.telegram.sendMessage(Number(admin.telegramId), `Nuevo empleado quiere registrarse:\nNombre: ${firstName}\nUsername: @${username || 'N/A'}\nTelegram ID: ${telegramId}\n\n¿Qué rol deseas asignarle?`, adminKeyboard);
            }
            catch (e) {
...
bot.action(/reject_(.+)/, async (ctx) => {
    ...
    try {
        ...
        await ctx.telegram.sendMessage(Number(updatedUser.telegramId), 'Lo sentimos, tu solicitud ha sido rechazada por el administrador.', telegraf_1.Markup.removeKeyboard());
    }
    catch (err) {
        console.error(err);
...
bot.command('admin_panel', (0, middleware_1.requireRole)(['ADMIN']), async (ctx) => {
    await ctx.reply('⚙️ *Panel de Administración*\nSelecciona una opción:', { parse_mode: 'MarkdownV2', ...getAdminKeyboard() });
});
// Admin Panel Callbacks
bot.action('admin_roles', async (ctx) => {
    try {
        const users = await db_1.prisma.user.findMany({ where: { role: { notIn: ['ADMIN', 'CLIENTE'] } } });
        if (users.length === 0) {
            await ctx.editMessageText('No hay usuarios para gestionar.');
            return;
        }
        const buttons = users.map(u => [telegraf_1.Markup.button.callback(`${u.firstName} - ${u.role}`, `manage_user_${u.id}`)]);
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_panel_back')]);
        await ctx.editMessageText('Selecciona un usuario para gestionar su rol:', telegraf_1.Markup.inlineKeyboard(buttons));
    }
    catch (err) {
        console.error(err);
...
bot.action(/manage_user_(.+)/, async (ctx) => {
    ...
    try {
        ...
        const keyboard = telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('Asignar MESERO', `set_role_MESERO_${userId}`)],
            [telegraf_1.Markup.button.callback('Asignar CONTADOR', `set_role_CONTADOR_${userId}`)],
            [telegraf_1.Markup.button.callback('RECHAZAR', `set_role_REJECTED_${userId}`)],
            [telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_roles')]
        ]);
...
bot.action(/set_role_(.+)_(.+)/, async (ctx) => {
    ...
    try {
        await db_1.prisma.user.update({ where: { id: userId }, data: { role: newRole } });
        await ctx.answerCbQuery(`Rol de usuario actualizado a ${newRole}`);
        // Return to roles menu
        const users = await db_1.prisma.user.findMany({ where: { role: { notIn: ['ADMIN', 'CLIENTE'] } } });
        const buttons = users.map(u => [telegraf_1.Markup.button.callback(`${u.firstName} - ${u.role}`, `manage_user_${u.id}`)]);
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_panel_back')]);
        await ctx.editMessageText('Rol actualizado. Selecciona un usuario para gestionar:', telegraf_1.Markup.inlineKeyboard(buttons));
    }
    catch (err) {
        console.error(err);
...
bot.action('admin_clientes', async (ctx) => {
    try {
        const clients = await db_1.prisma.user.findMany({ where: { role: 'CLIENTE' }, orderBy: { firstName: 'asc' } });
        if (clients.length === 0) {
            await ctx.editMessageText('No hay clientes registrados.', telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_panel_back')]
            ]));
            return;
        }
        const buttons = clients.map(c => [telegraf_1.Markup.button.callback(`👤 ${c.firstName} (${c.phone || 'Sin tel'})`, `admin_view_client_${c.id}`)]);
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_panel_back')]);
...
bot.action(/admin_view_client_(.+)/, async (ctx) => {
    ...
    try {
        ...
        if (!client)
            return ctx.answerCbQuery('Cliente no encontrado');
        ...
        const buttons = [
            [telegraf_1.Markup.button.callback('⬇️ Descargar Historial (CSV)', `export_client_csv_${client.id}`)],
            [telegraf_1.Markup.button.callback('❌ Eliminar Cliente', `delete_client_${client.id}`)],
            [telegraf_1.Markup.button.callback('⬅️ Volver a Lista', 'admin_clientes')]
        ];
        await ctx.editMessageText(msg, { parse_mode: 'MarkdownV2', ...telegraf_1.Markup.inlineKeyboard(buttons) });
    }
    catch (err) {
        console.error(err);
...
bot.action(/export_client_csv_(.+)/, async (ctx) => {
    ...
    try {
        ...
        if (!client)
            return ctx.answerCbQuery('Cliente no encontrado');
        if (client.clientOrders.length === 0)
            return ctx.answerCbQuery('Este cliente no tiene historial de pedidos.', { show_alert: true });
        const rows = [];
        ...
        const parser = new json2csv_1.Parser({ fields: ['Fecha', 'Mesa', 'Total', 'Metodo Pago', 'Productos'] });
...
bot.action(/delete_client_(.+)/, async (ctx) => {
    const clientId = ctx.match[1];
    try {
        await db_1.prisma.user.delete({ where: { id: clientId } });
        ...
        const buttons = clients.map(c => [telegraf_1.Markup.button.callback(`❌ Eliminar: ${c.firstName} (${c.phone || 'Sin tel'})`, `delete_client_${c.id}`)]);
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_panel_back')]);
        await ctx.editMessageText('👥 *Gestión de Clientes*\nSelecciona un cliente para eliminar su registro:', { parse_mode: 'Markdown', ...telegraf_1.Markup.inlineKeyboard(buttons) });
    }
    catch (err) {
        console.error(err);
...
bot.action('admin_mesas', async (ctx) => {
    try {
        const tables = await db_1.prisma.table.findMany({ orderBy: { number: 'asc' } });
        const buttons = tables.map(t => [telegraf_1.Markup.button.callback(`Mesa ${t.number} - ${t.status}`, `manage_mesa_${t.id}`)]);
        buttons.push([telegraf_1.Markup.button.callback('➕ Añadir Mesa', 'add_mesa')]);
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_panel_back')]);
        await ctx.editMessageText('Gestión de Mesas:', telegraf_1.Markup.inlineKeyboard(buttons));
    }
    catch (err) {
        console.error(err);
...
bot.action('add_mesa', async (ctx) => {
    try {
        const tables = await db_1.prisma.table.findMany({ orderBy: { number: 'desc' }, take: 1 });
        const nextNumber = tables.length > 0 ? tables[0].number + 1 : 1;
        await db_1.prisma.table.create({ data: { number: nextNumber } });
        await ctx.answerCbQuery(`Mesa ${nextNumber} añadida.`);
        // Reload mesas
        const allTables = await db_1.prisma.table.findMany({ orderBy: { number: 'asc' } });
        const buttons = allTables.map(t => [telegraf_1.Markup.button.callback(`Mesa ${t.number} - ${t.status}`, `manage_mesa_${t.id}`)]);
        buttons.push([telegraf_1.Markup.button.callback('➕ Añadir Mesa', 'add_mesa')]);
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_panel_back')]);
        await ctx.editMessageText('Gestión de Mesas:', telegraf_1.Markup.inlineKeyboard(buttons));
    }
...
bot.action(/manage_mesa_(.+)/, async (ctx) => {
    ...
    try {
        ...
        if (!table)
            return ctx.answerCbQuery('Mesa no encontrada');
        const keyboard = telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('❌ Eliminar Mesa', `delete_mesa_${tableId}`)],
            [telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_mesas')]
        ]);
...
bot.action(/delete_mesa_(.+)/, async (ctx) => {
    const tableId = ctx.match[1];
    try {
        await db_1.prisma.table.delete({ where: { id: tableId } });
...
bot.action('admin_cuentas', async (ctx) => {
    try {
        const openOrders = await db_1.prisma.order.findMany({
            where: { status: 'OPEN' },
            include: { table: true, user: true, client: true }
        });
        const userRole = await db_1.prisma.user.findUnique({ where: { telegramId: ctx.from.id } });
        if (openOrders.length === 0) {
            const btns = userRole?.role === 'ADMIN' ? [[telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_panel_back')]] : [];
            await ctx.editMessageText('No hay cuentas abiertas actualmente.', telegraf_1.Markup.inlineKeyboard(btns));
            return;
        }
        const buttons = openOrders.map(o => {
            ...
            return [telegraf_1.Markup.button.callback(`${loc} - ${orderDisplayId} ($${o.total.toFixed(2)})`, `view_order_${o.id}`)];
        });
...
bot.action(/view_order_(.+)/, async (ctx) => {
    ...
    try {
        ...
        if (!order)
            return ctx.answerCbQuery('Cuenta no encontrada');
        ...
        const inlineKeyboard = [
            [telegraf_1.Markup.button.callback('➕ Agregar Productos', `add_items_${order.id}`)],
            [telegraf_1.Markup.button.callback('✏️ Editar Cuenta (Eliminar)', `edit_order_${order.id}`)]
        ];
        // Specific button for App pickup orders to notify the client
        if (!order.tableId && order.clientId) {
            inlineKeyboard.push([telegraf_1.Markup.button.callback('🔔 Notificar Listo a Cliente', `notify_ready_${order.id}`)]);
        }
        inlineKeyboard.push([telegraf_1.Markup.button.callback('❌ Cerrar Cuenta (Cobrar)', `close_order_${order.id}`)]);
        inlineKeyboard.push([telegraf_1.Markup.button.callback('⬅️ Volver a Cuentas', `admin_cuentas`)]);
        await ctx.editMessageText(msg, {
            parse_mode: 'MarkdownV2',
            reply_markup: telegraf_1.Markup.inlineKeyboard(inlineKeyboard).reply_markup
        });
    }
    catch (err) {
        console.error(err);
...
bot.action(/notify_ready_(.+)/, async (ctx) => {
    ...
    try {
        ...
        await ctx.telegram.sendMessage(Number(order.client.telegramId), msgBody, { parse_mode: 'MarkdownV2' });
...
bot.action('admin_menu', async (ctx) => {
    productAddState.delete(ctx.from.id);
    try {
        const products = await db_1.prisma.product.findMany({ orderBy: { name: 'asc' } });
        const buttons = products.map(p => [telegraf_1.Markup.button.callback(`${p.name} - $${p.price.toFixed(2)}`, `manage_product_${p.id}`)]);
        buttons.push([telegraf_1.Markup.button.callback('➕ Añadir Producto', 'add_product')]);
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_panel_back')]);
        await ctx.editMessageText('🍔 Gestión de Menú:', telegraf_1.Markup.inlineKeyboard(buttons));
    }
    catch (err) {
        console.error(err);
...
bot.action(/manage_product_(.+)/, async (ctx) => {
    ...
    try {
        ...
        if (!product)
            return ctx.answerCbQuery('Producto no encontrado');
        const keyboard = telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('❌ Eliminar Producto', `delete_product_${productId}`)],
            [telegraf_1.Markup.button.callback('⬅️ Volver al menú', 'admin_menu')]
        ]);
...
bot.action('admin_finanzas', async (ctx) => {
    try {
        ...
        const closedOrders = await db_1.prisma.order.findMany({
            where: { status: 'CLOSED', closedAt: { gte: today } }
        });
        const expenses = await db_1.prisma.expense.findMany({
            where: { createdAt: { gte: today } }
        });
        const totalIncome = closedOrders.reduce((acc, curr) => acc + curr.total, 0);
        ...
        const keyboard = telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('➕ Registrar Gasto', 'admin_add_expense')],
            [telegraf_1.Markup.button.callback('📋 Ver Gastos de Hoy', 'admin_view_expenses')],
            [telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_panel_back')]
        ]);
...
bot.action('admin_view_expenses', async (ctx) => {
    try {
        ...
        const keyboard = telegraf_1.Markup.inlineKeyboard([[telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_finanzas')]]);
        await ctx.editMessageText(msg, { parse_mode: 'MarkdownV2', ...keyboard });
    }
    catch (err) {
        console.error(err);
...
bot.action('admin_config', async (ctx) => {
    try {
        const config = await db_1.prisma.restaurantConfig.upsert({
            where: { id: 1 },
            update: {},
            create: {}
        });
        ...
        await ctx.editMessageText(`🖨️ *Configuración del Ticket*\n\n*Logo URL:* ${escapeMarkdownV2(logo)}\n*Slogan/Ubicación:* ${escapeMarkdownV2(loc)}\n*Mensaje:* ${escapeMarkdownV2(msg)}`, {
            ...
            ...telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('✏️ Editar Logo', 'edit_config_logo')],
                [telegraf_1.Markup.button.callback('✏️ Editar Ubicación', 'edit_config_loc')],
                [telegraf_1.Markup.button.callback('✏️ Editar Mensaje', 'edit_config_msg')],
                [telegraf_1.Markup.button.callback('⬅️ Volver', 'admin_panel_back')]
            ])
        });
    }
...
async function handleSalesReport(ctx) {
    try {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const closedOrders = await db_1.prisma.order.findMany({
        ...
        const keyboard = telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('⬇️ Exportar CSV', 'export_sales_csv')]
        ]);
...
bot.command('sales_report', (0, middleware_1.requireRole)(['ADMIN', 'CONTADOR']), handleSalesReport);
bot.hears('📊 Reporte de Ventas', (0, middleware_1.requireRole)(['ADMIN', 'CONTADOR']), handleSalesReport);
async function generateAndSendSalesCSV(ctx, date) {
    try {
        ...
        if (closedOrders.length === 0) {
            ...
            if (ctx.answerCbQuery)
                await ctx.answerCbQuery(`No hay ventas el ${dateStr} para exportar.`, { show_alert: true });
            else
                await ctx.reply(`No hay ventas el ${dateStr} para exportar.`);
            return;
        }
        ...
        if (ctx.answerCbQuery)
            ...
            await ctx.reply('Error al exportar reporte.');
    }
}
...
async function handleNewOrderFlow(ctx) {
    try {
        const tables = await db_1.prisma.table.findMany({ orderBy: { number: 'asc' } });
        if (tables.length === 0) {
            return ctx.reply('No hay mesas configuradas en el sistema. Usa Panel de Administración para agregar mesas primero.');
        }
        const buttons = tables.map(t => [telegraf_1.Markup.button.callback(`Mesa ${t.number} - ${t.status === 'AVAILABLE' ? 'Libre' : 'Ocupada'}`, `select_table_${t.id}`)]);
...
bot.command('new_order', (0, middleware_1.requireRole)(['ADMIN', 'MESERO']), handleNewOrderFlow);
bot.hears('📝 Nueva Orden / Ver Mesa', (0, middleware_1.requireRole)(['ADMIN', 'MESERO']), handleNewOrderFlow);
bot.hears('🥡 Para Llevar', (0, middleware_1.requireRole)(['ADMIN', 'MESERO']), async (ctx) => {
    const telegramId = ctx.from.id;
    try {
        const user = await db_1.prisma.user.findUnique({ where: { telegramId: BigInt(telegramId) } });
        if (!user)
        ...
        const buttons = products.map(p => [telegraf_1.Markup.button.callback(`➕ ${p.name} ($${p.price.toFixed(2)})`, `add_prod_${p.id}`)]);
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver al menú principal', 'main_menu')]);
...
bot.hears('🧾 Cuentas Abiertas', (0, middleware_1.requireRole)(['ADMIN', 'MESERO']), async (ctx) => {
    try {
        const openOrders = await db_1.prisma.order.findMany({
            where: { status: 'OPEN' },
            include: { table: true, user: true, client: true }
...
bot.hears('⚙️ Panel de Administración', (0, middleware_1.requireRole)(['ADMIN']), async (ctx) => {
...
async function helpCommandLogic(ctx) {
    const telegramId = ctx.from.id;
    try {
        const user = await db_1.prisma.user.findUnique({ where: { telegramId } });
        if (!user) {
...
bot.action('list_tables', async (ctx) => {
    try {
        ...
        const buttons = tables.map(t => [telegraf_1.Markup.button.callback(`Mesa ${t.number} - ${t.status === 'AVAILABLE' ? 'Libre' : 'Ocupada'}`, `select_table_${t.id}`)]);
        await ctx.editMessageText('🍽️ *Nueva Orden / Ver Cuenta*\nSelecciona una mesa:', { parse_mode: 'MarkdownV2', ...telegraf_1.Markup.inlineKeyboard(buttons) });
    }
    catch (err) {
        console.error(err);
        await ctx.answerCbQuery('Error al cargar mesas.');
...
bot.action(/select_table_(.+)/, async (ctx) => {
    ...
    try {
        ...
        if (table.orders.length === 0) {
            ...
            await db_1.prisma.table.update({ where: { id: table.id }, data: { status: 'OCCUPIED' } });
            await ctx.editMessageText(`✅ Cuenta abierta en *Mesa ${escapeMarkdownV2(table.number)}* por ${escapeMarkdownV2(user.firstName)}\\.\nUsa el botón abajo para agregar productos\\.`, {
                parse_mode: 'MarkdownV2',
                ...telegraf_1.Markup.inlineKeyboard([
                    [telegraf_1.Markup.button.callback('➕ Agregar Productos', `add_items_${newOrder.id}`)],
                    [telegraf_1.Markup.button.callback('⬅️ Volver a Todas las Mesas', 'list_tables')]
                ])
            });
        }
        else {
            ...
            await ctx.editMessageText(msg, {
                ...
                ...telegraf_1.Markup.inlineKeyboard([
                    [telegraf_1.Markup.button.callback('➕ Agregar Productos', `add_items_${order.id}`)],
                    [telegraf_1.Markup.button.callback('👤 Asignar Cliente', `assign_client_${order.id}`)],
                    [telegraf_1.Markup.button.callback('✏️ Editar Cuenta (Eliminar)', `edit_order_${order.id}`)],
                    [telegraf_1.Markup.button.callback('❌ Cerrar Cuenta (Cobrar)', `close_order_${order.id}`)],
                    [telegraf_1.Markup.button.callback('⬅️ Volver a Todas las Mesas', 'list_tables')]
                ])
            });
        }
...
bot.action(/edit_order_(.+)/, async (ctx) => {
    ...
    try {
        ...
        if (!order || !['OPEN', 'CART'].includes(order.status))
            return ctx.answerCbQuery('La cuenta no está abierta o no existe');
        const loc = order.table ? `Mesa ${escapeMarkdownV2(order.table.number)}` : 'App';
        ...
        if (order.items.length === 0) {
            ...
            for (const item of order.items) {
                ...
                buttons.push([telegraf_1.Markup.button.callback(`❌ Eliminar ${itemName} ($${item.price.toFixed(2)})`, `rm_itm_${item.id}`)]);
            }
        }
        const backCb = order.table ? `select_table_${order.table.id}` : `view_client_order_${order.id}`;
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver a la cuenta', backCb)]);
        await ctx.editMessageText(msg, {
            parse_mode: 'MarkdownV2',
            reply_markup: telegraf_1.Markup.inlineKeyboard(buttons).reply_markup
        });
...
bot.action(/rm_itm_(.+)/, async (ctx) => {
    ...
    try {
        const item = await db_1.prisma.orderItem.findUnique({ where: { id: itemId }, include: { order: { include: { table: true } } } });
        if (!item)
            return ctx.answerCbQuery('El producto ya fue eliminado', { show_alert: true });
        ...
        if (item.quantity > 1) {
            ...
            // Delete item
            await db_1.prisma.orderItem.delete({ where: { id: itemId } });
        }
        // Update order total
        ...
        if (updatedOrder.items.length === 0) {
            ...
            for (const currentItem of updatedOrder.items) {
                ...
                buttons.push([telegraf_1.Markup.button.callback(`❌ Eliminar ${itemName} ($${currentItem.price.toFixed(2)})`, `rm_itm_${currentItem.id}`)]);
            }
        }
...
bot.action(/assign_client_(.+)/, async (ctx) => {
    const orderId = ctx.match[1];
    try {
        const order = await db_1.prisma.order.findUnique({ where: { id: orderId }, include: { table: true } });
        if (!order || !['OPEN', 'CART'].includes(order.status))
            return ctx.answerCbQuery('La cuenta no está abierta o no existe');
        const backCb = order.table ? `select_table_${order.table.id}` : `view_client_order_${order.id}`;
        const loc = order.table ? `Mesa ${order.table.number}` : 'App';
        const buttons = [
            [telegraf_1.Markup.button.callback('🔍 Buscar Cliente (Nombre/Tel)', `search_client_${order.id}`)],
            [telegraf_1.Markup.button.callback('📋 Listar Todos los Clientes', `list_all_clients_${order.id}`)]
        ];
        if (order.clientId) {
            buttons.push([telegraf_1.Markup.button.callback('❌ Quitar Cliente Actual', `set_client_${order.id}_none`)]);
        }
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver a la cuenta', backCb)]);
...
bot.action(/search_client_(.+)/, async (ctx) => {
    ...
    await ctx.editMessageText('🔎 *Buscar Cliente*\nEscribe el nombre o teléfono del cliente para buscarlo:', {
        parse_mode: 'MarkdownV2',
        ...telegraf_1.Markup.inlineKeyboard([[telegraf_1.Markup.button.callback('❌ Cancelar', `assign_client_${orderId}`)]])
    });
});
bot.action(/list_all_clients_(.+)/, async (ctx) => {
    const orderId = ctx.match[1];
    try {
        const clients = await db_1.prisma.user.findMany({ where: { role: 'CLIENTE' }, orderBy: { firstName: 'asc' }, take: 50 });
        if (clients.length === 0) {
            return ctx.answerCbQuery('No hay clientes registrados.', { show_alert: true });
        }
        const buttons = clients.map(c => [telegraf_1.Markup.button.callback(`👤 ${c.firstName} (${c.phone || 'N/T'})`, `set_client_${orderId}_${c.id}`)]);
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver', `assign_client_${orderId}`)]);
        await ctx.editMessageText('👥 *Todos los Clientes:*', telegraf_1.Markup.inlineKeyboard(buttons));
    }
    catch (err) {
        console.error(err);
...
bot.action(/set_client_(.+)_(.+)/, async (ctx) => {
    ...
    try {
        const order = await db_1.prisma.order.findUnique({ where: { id: orderId }, include: { table: true } });
        if (!order)
            return ctx.answerCbQuery('Orden no encontrada');
        if (clientId === 'none') {
            await db_1.prisma.order.update({ where: { id: orderId }, data: { clientId: null } });
            await ctx.answerCbQuery('Cliente quitado de la orden');
        }
        else {
            await db_1.prisma.order.update({ where: { id: orderId }, data: { clientId } });
        ...
        if (order.tableId) {
            ...
            await ctx.editMessageText('✅ Actualizado. Vuelve a abrir la mesa.', {
                ...telegraf_1.Markup.inlineKeyboard([
                    [telegraf_1.Markup.button.callback(`Volver a la Mesa ${order.table?.number || ''}`, `select_table_${tableId}`)]
                ])
            });
        }
        else {
            await ctx.editMessageText('✅ Cliente asignado.', {
                ...telegraf_1.Markup.inlineKeyboard([
                    [telegraf_1.Markup.button.callback(`Volver a la Orden`, `view_client_order_${order.id}`)]
                ])
            });
        }
    }
...
bot.action(/add_items_(.+)/, async (ctx) => {
    ...
    try {
        ...
        const buttons = products.map(p => [telegraf_1.Markup.button.callback(`➕ ${p.name} ($${p.price.toFixed(2)})`, `add_prod_${p.id}`)]);
        buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver a la cuenta', backCb)]);
        await ctx.editMessageText(`📖 *Menú*\nSelecciona productos para ${loc}:`, { parse_mode: 'MarkdownV2', ...telegraf_1.Markup.inlineKeyboard(buttons) });
    }
    catch (err) {
        console.error(err);
...
bot.action(/add_prod_(.+)/, async (ctx) => {
    ...
    if (!state)
        return ctx.answerCbQuery('Sesión caducada, por favor vuelve a abrir la mesa.', { show_alert: true });
    const orderId = state.orderId;
    try {
        const product = await db_1.prisma.product.findUnique({ where: { id: productId } });
        const order = await db_1.prisma.order.findUnique({ where: { id: orderId }, include: { table: true } });
        if (!product || !order)
            return ctx.answerCbQuery('Error, producto u orden no encontrados');
        if (order.table && order.table.status === 'BLOCKED') {
        ...
        const existingItem = await db_1.prisma.orderItem.findFirst({
...
bot.action(/close_order_(.+)/, async (ctx) => {
    ...
    try {
        const order = await db_1.prisma.order.findUnique({ where: { id: orderId }, include: { table: true } });
        if (!order)
            return ctx.answerCbQuery('No se encontró la orden');
        const loc = order.table ? `Mesa ${escapeMarkdownV2(order.table.number)}` : 'Pedido App';
        const backCb = order.table ? `select_table_${order.table.id}` : `view_client_order_${order.id}`;
        await ctx.editMessageText(`💰 *Cobrar ${loc}*\n*Total:* \\$${escapeMarkdownV2(order.total.toFixed(2))}\n\n¿Método de pago?`, {
            parse_mode: 'MarkdownV2',
            ...telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('💵 Efectivo', `pay_cash_${order.id}`)],
                [telegraf_1.Markup.button.callback('💳 Tarjeta', `pay_card_${order.id}`)],
                [telegraf_1.Markup.button.callback('⬅️ Volver a la cuenta', backCb)]
            ])
        });
    }
    catch (err) {
...
async function processPaymentAndCloseOrder(ctx, orderId, telegramId, method) {
    const order = await db_1.prisma.order.findUnique({ where: { id: orderId }, include: { table: true, items: true, client: true } });
    const user = await db_1.prisma.user.findUnique({ where: { telegramId } });
    if (!order || !user) {
        if (ctx.answerCbQuery)
            await ctx.answerCbQuery('Error al identificar usuario u orden');
        else
            await ctx.reply('Error al identificar usuario u orden');
        return;
    }
    const config = await db_1.prisma.restaurantConfig.upsert({ where: { id: 1 }, update: {}, create: {} });
    await db_1.prisma.order.update({
        where: { id: orderId },
        data: { status: 'CLOSED', paymentMethod: method, closedById: user.id, closedAt: new Date() }
    });
    if (order.tableId) {
        await db_1.prisma.table.update({ where: { id: order.tableId }, data: { status: 'AVAILABLE' } });
    ...
    if (ctx.updateType === 'callback_query' && ctx.editMessageText) {
        await ctx.editMessageText(msg, { parse_mode: 'MarkdownV2', ...telegraf_1.Markup.inlineKeyboard([[telegraf_1.Markup.button.callback(cbText, cbBack)]]) });
    }
    else {
        await ctx.reply(msg, { parse_mode: 'MarkdownV2', ...telegraf_1.Markup.inlineKeyboard([[telegraf_1.Markup.button.callback(cbText, cbBack)]]) });
    }
    try {
        ...
        if (order.client && order.client.telegramId) {
            try {
                ...
                await ctx.telegram.sendPhoto(Number(order.client.telegramId), { source: imageBuffer }, {
    ...
    try {
        ...
        if (order?.pickupCode && order?.status === 'OPEN') {
            ...
            return ctx.editMessageText('🔒 *Validación de Entrega*\n\nPor favor, pídele al cliente e ingresa aquí su *código PIN de 4 dígitos* para poder cobrar y entregar su pedido:', {
                parse_mode: 'Markdown',
                ...telegraf_1.Markup.inlineKeyboard([[telegraf_1.Markup.button.callback('❌ Cancelar', `view_order_${orderId}`)]])
            });
        }
...
bot.on('message', async (ctx, next) => {
    ...
    if (ctx.message && 'text' in ctx.message) {
        ...
        if (expenseState.has(telegramId)) {
            ...
            if (state.step === 'DESCRIPTION') {
                ...
                try {
                    ...
                    const keyboard = telegraf_1.Markup.inlineKeyboard([[telegraf_1.Markup.button.callback('📊 Volver a Finanzas', 'admin_finanzas')]]);
        ...
        if (waiterClientSearchState.has(telegramId)) {
            ...
            try {
                const clients = await db_1.prisma.user.findMany({
                    where: {
                        role: 'CLIENTE',
                        OR: [
                ...
                if (clients.length === 0) {
                    await ctx.reply(`❌ No se encontraron clientes que coincidan con "${escapeMarkdownV2(text)}"\\.`, {
                        parse_mode: 'MarkdownV2',
                        ...telegraf_1.Markup.inlineKeyboard([
                            [telegraf_1.Markup.button.callback('🔍 Reintentar', `search_client_${state.orderId}`)],
                            [telegraf_1.Markup.button.callback('📋 Ver Todos', `list_all_clients_${state.orderId}`)],
                            [telegraf_1.Markup.button.callback('⬅️ Volver', `assign_client_${state.orderId}`)]
                        ])
                    });
                    return;
                }
                const buttons = clients.map(c => [telegraf_1.Markup.button.callback(`👤 ${c.firstName} (${c.phone || 'N/T'})`, `set_client_${state.orderId}_${c.id}`)]);
                buttons.push([telegraf_1.Markup.button.callback('⬅️ Volver', `assign_client_${state.orderId}`)]);
        ...
        if (waiterPickupCodeState.has(telegramId)) {
            ...
            const order = await db_1.prisma.order.findUnique({ where: { id: state.orderId } });
        ...
        if (configEditState.has(telegramId)) {
            ...
            try {
                if (editMode === 'LOGO') {
                    await db_1.prisma.restaurantConfig.upsert({ where: { id: 1 }, update: { logoUrl: text }, create: { logoUrl: text } });
                    await ctx.reply('Logo actualizado exitosamente.');
                }
                else if (editMode === 'LOCATION') {
                    await db_1.prisma.restaurantConfig.upsert({ where: { id: 1 }, update: { locationText: text }, create: { locationText: text } });
                    await ctx.reply('Ubicación actualizada exitosamente.');
                }
                else if (editMode === 'MESSAGE') {
                    await db_1.prisma.restaurantConfig.upsert({ where: { id: 1 }, update: { thankYouMessage: text }, create: { thankYouMessage: text } });
...
bot.hears('🚪 Salir', (0, middleware_1.requireRole)(['CLIENTE', 'ADMIN', 'MESERO', 'CONTADOR']), async (ctx) => {
    await ctx.reply('Has cerrado tu sesión actual en el bot.\n\n¿Cómo deseas registrarte ahora?', telegraf_1.Markup.keyboard([
        ['🌮 Soy Cliente', '💼 Soy Empleado']
    ]).oneTime().resize());
});
bot.hears('📖 Ver Menú', (0, middleware_1.requireRole)(['CLIENTE', 'ADMIN']), async (ctx) => {
    try {
        const products = await db_1.prisma.product.findMany({ orderBy: { name: 'asc' } });
        if (products.length === 0) {
...
bot.hears('📋 Mis Pedidos', (0, middleware_1.requireRole)(['CLIENTE', 'ADMIN']), async (ctx) => {
    const telegramId = ctx.from.id;
    try {
        const user = await db_1.prisma.user.findUnique({ where: { telegramId } });
        if (!user)
...
bot.hears('🛍️ Hacer Pedido', (0, middleware_1.requireRole)(['CLIENTE', 'ADMIN']), async (ctx) => {
    const telegramId = ctx.from.id;
    try {
        const user = await db_1.prisma.user.findUnique({ where: { telegramId } });
        if (!user)
        ...
        let order = await db_1.prisma.order.findFirst({
            where: {
                clientId: user.id,
        ...
        if (!order) {
            order = await db_1.prisma.order.create({
                data: {
                    clientId: user.id,
                    status: 'CART',
                    total: 0
        ...
        const keyboard = telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('➕ Agregar Productos', `add_items_${order.id}`)],
            order.items.length > 0 ? [telegraf_1.Markup.button.callback('✏️ Editar (Eliminar)', `edit_order_${order.id}`)] : [],
            order.items.length > 0 ? [telegraf_1.Markup.button.callback('✅ Enviar Pedido', `submit_client_order_${order.id}`)] : [],
            [telegraf_1.Markup.button.callback('❌ Vaciar Carrito', `empty_cart_${order.id}`)]
...
bot.action(/submit_client_order_(.+)/, async (ctx) => {
    ...
    try {
        ...
        const btnKeyboard = telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('👀 Ver Pedido Confirmar', `waiter_view_pending_${order.id}`)]
        ]);
        for (const user of staff) {
            try {
                await ctx.telegram.sendMessage(Number(user.telegramId), `🛎️ ¡Nuevo pedido recibido\\!\n*ID Orden:* ${escapeMarkdownV2(orderDisplayId)}\n*Cliente:* ${clientName}\n*Total:* \\$${total}`, { parse_mode: 'MarkdownV2', ...btnKeyboard });
            }
            catch (e) {
...
bot.action(/empty_cart_(.+)/, async (ctx) => {
    ...
    try {
        ...
        await db_1.prisma.orderItem.deleteMany({ where: { orderId } });
        await db_1.prisma.order.delete({ where: { id: orderId } });
        await ctx.editMessageText('🗑️ Tu carrito ha sido vaciado.', telegraf_1.Markup.inlineKeyboard([
            [telegraf_1.Markup.button.callback('📖 Ver Menú', 'view_menu_client')]
        ]));
    }
    catch (err) {
        console.error(err);
...
bot.hears('🛎️ Pedidos de Clientes', (0, middleware_1.requireRole)(['ADMIN', 'MESERO']), async (ctx) => {
    try {
        ...
        const buttons = pendingOrders.map(o => {
            ...
            return [telegraf_1.Markup.button.callback(`🕒 ${clientName} - $${o.total.toFixed(2)}`, `waiter_view_pending_${o.id}`)];
        });
...
bot.action(/waiter_view_pending_(.+)/, async (ctx) => {
    ...
    try {
        ...
        await ctx.editMessageText(msg, {
            parse_mode: 'MarkdownV2',
            ...telegraf_1.Markup.inlineKeyboard([
                [telegraf_1.Markup.button.callback('✅ Confirmar Recepción', `waiter_accept_order_${order.id}`)],
                [telegraf_1.Markup.button.callback('⬅️ Volver a Lista', 'btn_view_pending_list')]
            ])
        });
    }
    catch (err) {
...
bot.action(/waiter_accept_order_(.+)/, async (ctx) => {
    ...
    const telegramId = ctx.from.id;
    try {
        const waiter = await db_1.prisma.user.findUnique({ where: { telegramId } });
        if (!waiter)
            return ctx.answerCbQuery('Personal no autorizado');
        const order = await db_1.prisma.order.findUnique({ where: { id: orderId }, include: { client: true } });
        if (!order || order.status !== 'PENDING_APPROVAL') {
            return ctx.answerCbQuery('El pedido ya fue procesado o no existe.', { show_alert: true });
        }
        await db_1.prisma.order.update({
        ...
        if (order.client && order.client.telegramId) {
            try {
                await ctx.telegram.sendMessage(Number(order.client.telegramId), `✅ *¡Tu pedido ha sido recibido y se está preparando\\!*\nSerás notificado por el personal de cualquier actualización\\.`, { parse_mode: 'MarkdownV2' });
            }
            catch (e) {
...
app.get('/api/status', async (req, res) => {
    try {
        const activeTables = await db_1.prisma.table.count({ where: { status: 'OCCUPIED' } });
        const clientOrders = await db_1.prisma.order.count({
            where: {
                tableId: null,
                clientId: { not: null },
                status: { in: ['OPEN', 'PENDING_APPROVAL'] }
        ...
        const toGoOrders = await db_1.prisma.order.count({
            where: {
                tableId: null,
                clientId: null,
                status: { in: ['OPEN', 'PENDING_APPROVAL'] }
...
app.get('/api/tables', async (req, res) => {
    try {
        const tables = await db_1.prisma.table.findMany({
            ...
            include: {
                orders: {
                    where: { status: { in: ['OPEN', 'PENDING_APPROVAL', 'CART'] } },
                    include: {
                        user: true,
                        items: {
                            include: { user: true }
        ...
        const formattedTables = tables.map((t) => {
            ...
            if (activeOrder) {
                // Primary waiter
                if (activeOrder.user)
                    waiters.push(activeOrder.user.firstName);
                // Contributors
...
app.post('/api/tables', async (req, res) => {
    try {
        const lastTable = await db_1.prisma.table.findFirst({
            orderBy: { number: 'desc' }
        });
...
app.patch('/api/tables/:id/status', async (req, res) => {
    try {
        ...
        const table = await db_1.prisma.table.findUnique({
            where: { id },
            include: { orders: { where: { status: { in: ['OPEN', 'PENDING_APPROVAL'] } } } }
        });
        if (!table)
...
app.delete('/api/tables/:id', async (req, res) => {
    try {
        ...
        const activeOrder = await db_1.prisma.order.findFirst({
            where: { tableId: id, status: { in: ['OPEN', 'PENDING_APPROVAL'] } }
        });
        if (activeOrder) {
...
app.get('/api/sales', async (req, res) => {
    try {
        ...
        const where = { status: 'CLOSED', closedAt: { gte: start, lte: end } };
        const summary = await db_1.prisma.order.aggregate({
            _sum: { total: true },
            where
        });
        const history = await db_1.prisma.order.findMany({
            where,
            include: {
                table: true,
            ...
            try {
                if (imageUrl) {
                    ...
                    if (imageUrl.startsWith('/uploads/')) {
                        photoSource = { source: path_1.default.join(__dirname, '../public', imageUrl) };
                    }
...
app.get('/api/ticket/:orderId', async (req, res) => {
    try {
        ...
        const [order, config] = await Promise.all([
            ...
            db_1.prisma.restaurantConfig.findFirst()
        ]);
        if (!order)
